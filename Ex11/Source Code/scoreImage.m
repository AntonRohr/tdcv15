function [ score_SSD, score_NCC ] = scoreImage( template, image )
...
%   the template in the image. 
    ...
    score_SSD = zeros(size(image) - size(template)); score_NCC = score_SSD;

    %% Loop over the image, compute scores
    for y=1 : size(score_SSD,1)-1
        for x=1 : size(score_SSD,2)-1
