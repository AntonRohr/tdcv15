...
%% Compute inliers
% this breaks if we compute the inliers of the image itself
for i = 2:size(sequence,3)
...
%% Save each correspondence image
for i = 2:size(sequence,3)
    figure = plotMatches(sequence(:,:,1),sequence(:,:,i),inliers_0{i},inliers_i{i});
    print(figure, ['correspondences/c_' num2str(i) ], '-dpng'); 
end