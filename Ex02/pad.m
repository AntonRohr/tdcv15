...
if strcmp(type,'clamp')
    ...
    % Corners
    padded_image(end-padY+1:end,end-padX+1:end) = uint8(ones(padX,padY))*input(end,end);
    padded_image(1:padX,1:padY) = uint8(ones(padX,padY))*input(1,1);
    padded_image(1:padX,end-padY+1:end) = uint8(ones(padX,padY))*input(1,end);
    padded_image(end-padX+1:end,1:padY) = uint8(ones(padX,padY))*input(end,1);
